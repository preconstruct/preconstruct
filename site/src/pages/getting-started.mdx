...
> A repository with the end result of this guide is available at https://github.com/preconstruct/a-random-number
...
🎁 ? a-random-number would you like preconstruct to automatically add `preconstruct dev` in a postinstall hook so that importing entrypoints during development will import the s
ource Yes
...
preconstruct has also added a script called `postinstall` which will be automatically run by package managers like Yarn and npm. `preconstruct dev` will create links from the dist files specified above to your source file so that your can import the directory that the package.json is in and you'll get the source files. While this is primarily useful in monorepos, it's also useful in single package repos because it allows you to run tests against your dist files but still run them against your source files in development for faster feedback loop.
