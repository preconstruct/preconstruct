...
preconstruct's core principle is that applying constraints that restrict what users can do can improve the implementation and usage of a tool.
...
- Solutions for multi-package repos scale down to single-package repos but the inverse is not true. Problems should therefore be solved with an emphasis on multi-package repos
