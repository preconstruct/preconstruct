...
We're going to setup a multi-package repo that uses Preconstruct to build its packages. We're also going to use [Yarn Workspaces](https://yarnpkg.com/en/docs/workspaces) for linking, Yarn Workspaces isn't the only multi-package repo linking tool that you can use with Preconstruct, [Lerna](https://github.com/lerna/lerna) and [Bolt](https://github.com/boltpkg/bolt) will also work.
