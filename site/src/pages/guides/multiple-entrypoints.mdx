...
> **Note:** this guide builds upon [the Getting Started guide](/getting-started) so if you haven't gone through it, you should do that first.

> A repository with the end result of this guide is available at https://github.com/preconstruct/a-random-number-with-create
...
<details>
  <summary>What does `"."` mean here?</summary>

`"."` is referring to the current directory so in this context it means that the current directory should be an entrypoint. So consumers should be able to import `a-random-number` (`"."`) and `a-random-number/create`
...
Since the source file for our `source` entrypoint isn't at the default of `src/index.js` relative to the entrypoint, we need to use the `source` option in the `package.json` in the `source` directory.
...
Now if we run `yarn preconstruct build`, everything will work 🎉
