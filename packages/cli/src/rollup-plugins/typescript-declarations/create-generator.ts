...
let getService = weakMemoize((project: Project) =>
  weakMemoize((typescript: Typescript) =>
    memoize(async (configFileName: string) => {
      ...
      if (thing.options.baseUrl === undefined) thing.options.baseUrl = ".";
      thing.options.paths = {
        ...thing.options.paths
      };
      for (let pkg of project.packages) {
        for (let entrypoint of pkg.entrypoints) {
          thing.options.paths[entrypoint.name] = [entrypoint.source];
        }
      }
...
export async function createDeclarationCreator(
  ...
  let { service, options } = await getService(pkg.project)(typescript)(
    configFileName
  );
