...
test("typescript with custom tsconfig filename in standalone package", async () => {
  let tmpPath = await testdir({
    "package.json": JSON.stringify({
...
test("typescript with invalid custom tsconfig filename in standalone package", async () => {
  let tmpPath = await testdir({
    "package.json": JSON.stringify({
  ...
  await expect(build(tmpPath)).rejects.toMatchInlineSnapshot(
    `[Error: 🎁 typescript-invalid-custom-config an entrypoint source file ends with the .ts or tsx extension but no TypeScript config exists, please create one.]`
  );
});

test("typescript with custom tsconfig filename in monorepo package", async () => {
  let tmpPath = await testdir({
    "package.json": JSON.stringify({
      name: "monorepo-root",
      private: true,
...
test("typescript with custom invalid tsconfig filename in monorepo package", async () => {
  let tmpPath = await testdir({
    "package.json": JSON.stringify({
      name: "monorepo-root",
      private: true,
  ...
  await expect(build(tmpPath)).rejects.toMatchInlineSnapshot(
    `[Error: 🎁 pkg-a an entrypoint source file ends with the .ts or tsx extension but no TypeScript config exists, please create one.]`
  );
});
